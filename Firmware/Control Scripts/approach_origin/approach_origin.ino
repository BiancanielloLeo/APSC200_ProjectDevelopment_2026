// =============================================================================
// APPROACH ORIGIN — turn-to-target, drive straight, turn-to-zero, verify
// =============================================================================
//
...
// unchanged from both.
//
// CYCLE (one call to approachOrigin(), driven from loop()):
//
//   1. Read camera (x, y, theta). If already within POSITION_TOLERANCE_M of
//      (0,0), stop and hold — done.
//   2. TURN   — point at (0,0). Same two-stage turn as heading_align.ino,
//               closed on encoders, camera only gives the target angle.
//   3. DRIVE  — straight line for the camera-measured distance, dead-reckoned
//               entirely on encoders using per-wheel feedforward + shared PI
//               velocity control (from pi_tune_test.ino). Camera is not
//               consulted during the drive.
//   4. TURN   — face 0 deg, same two-stage turn.
//   5. Stop, wait for the vision pipeline + an observe pause, then let a
//      fresh camera packet arrive.
//
// loop() calls this repeatedly. Step 1 on the NEXT call is the verification:
// if the drive undershot/overshot, it just becomes the start of another
// cycle. If it landed inside tolerance, step 1 stops it and holds.
//
// Move the robot away by hand; it drives itself back.
//
// =============================================================================
...
// =============================================================================
// APPROACH CYCLE — the new top-level behavior
// =============================================================================
// Called once per loop() iteration once the safety gates pass. Either:
//   - we're already home -> hold, return true
//   - we're not -> turn at target, drive, turn to zero, return false
...
static bool approachOrigin() {
    float x = currentX, y = currentY;
    float distToOrigin = sqrtf(x * x + y * y);

    Serial.printf("[pos] (%.3f, %.3f)  dist=%.3f m  hdg=%.2f deg\n",
                  x, y, distToOrigin, currentHeading * 180.0f / PI);

    if (distToOrigin <= POSITION_TOLERANCE_M) {
        allStop();
        Serial.println("[hold] within tolerance of (0,0)");
        return true;
    }

    // ── Stage 1: turn to point at the origin ────────────────────────────────
    float desiredHeadingDeg = atan2f(-y, -x) * 180.0f / PI;
    ...
    if (fabsf(aimErr) >= MIN_CORRECTION_DEG) {
        Serial.printf("[turn->target] want %.2f deg, err %.2f\n", desiredHeadingDeg, aimErr);
        executeTurn(aimErr);
    }

    // ── Stage 2: drive straight for the camera-measured distance ───────────
    driveStraightMeters(distToOrigin);

    // ── Check a fresh camera reading before deciding on Stage 3 ─────────────
    // No point squaring up to 0 deg if the drive undershot/overshot — that
    // heading gets thrown away next cycle anyway when we re-aim at the
    // origin. Only align to 0 once we've actually landed.
    uint32_t pktBefore = lastPacketNum;
    if (!waitForFreshPackets(pktBefore, FRESH_PACKET_TIMEOUT_MS) || !haveHeading) {
        allStop();
        return false;
    }

    float distAfterDrive = sqrtf(currentX * currentX + currentY * currentY);
    if (distAfterDrive > POSITION_TOLERANCE_M) {
        Serial.printf("       [skip turn->zero] dist=%.3f m, still outside tolerance\n",
                      distAfterDrive);
        allStop();
        return false;
    }

    // ── Stage 3: landed — turn to face 0 deg ─────────────────────────────
    float hdgDeg   = currentHeading * 180.0f / PI;
    float finalErr = wrapDeg(TARGET_HEADING_DEG - hdgDeg);
    if (fabsf(finalErr) >= MIN_CORRECTION_DEG) {
        Serial.printf("[turn->zero] err %.2f\n", finalErr);
        executeTurn(finalErr);
    }
...
static void printStatus() {
    ...
    Serial.printf("  goal: (0,0) +/- %.2f m, final heading %.0f deg\n",
                  POSITION_TOLERANCE_M, TARGET_HEADING_DEG);
...
void setup() {
    ...
    Serial.println("\n\n=== APPROACH ORIGIN (turn - drive - turn) ===");
    Serial.printf("Robot ID %d, target (0,0) +/- %.2f m, final heading %.0f deg\n",
                  THIS_ROBOT_ID, POSITION_TOLERANCE_M, TARGET_HEADING_DEG);
...
void loop() {
    ...
    // ── Already home? Hold and return without moving. ───────────────────────
    float distNow = sqrtf(currentX * currentX + currentY * currentY);
    if (distNow <= POSITION_TOLERANCE_M) {
        allStop();
        if (doPrint) {
            lastPrint = millis();
            Serial.printf("[hold] (%.3f, %.3f)  dist=%.3f m  hdg=%.2f deg\n",
                          currentX, currentY, distNow, currentHeading * 180.0f / PI);
        }
        delay(10);
        return;
    }

    // ── Run one full turn-drive-turn cycle ──────────────────────────────────
    uint32_t pktBefore = lastPacketNum;
    approachOrigin();
